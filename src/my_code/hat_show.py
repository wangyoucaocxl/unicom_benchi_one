...
if not NCS:
    ...
    model_xml = os.path.join(basepath1, "model_hat.xml") # xml文件路径
    model_bin = os.path.join(basepath1, "model_hat.bin") # bin文件路径
else:
    # 用神经棒（NCS）加速，选择FP16
    basepath2 = load_dict["basefile2"]    # 
    model_xml = os.path.join(basepath2, "model_hat.xml") # xml文件路径
    model_bin = os.path.join(basepath2, "model_hat.bin") # bin文件路径
...
if __name__ == "__main__":
    ...
    imagePath = "D:/7 input/data/3/20.jpg.jpga.jpg"
#    imagePath = "D:/7 input/data/4/2a056922de18c95c0ecc378bc2d2772e.jpg"
    
    image = cv2.imread(imagePath)
    for i in range(100):
        start = time.time()
        label =  predict_hat(image)
        end = time.time()
