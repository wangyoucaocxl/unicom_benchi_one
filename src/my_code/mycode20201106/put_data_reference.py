...
from src.shoe_view import predict_shoe
from src.hat_view import predict_hat
import numpy as np
import cv2
...
det_net = plugin1.load(network=net1)  # create an executable network

intel_models2 = "C:\\Intel\\computer_vision_sdk_2018.5.456\\deployment_tools\\intel_models\\face-detection-retail-0004\\FP32"
xml_path2 = os.path.join(intel_models2,"face-detection-retail-0004.xml")
bin_path2 = os.path.join(intel_models2,"face-detection-retail-0004.bin")
net2=IENetwork(model=xml_path2, weights=bin_path2)
input_layer2 = next(iter(net2.inputs))
n2, c2, h2, w2 = net2.inputs[input_layer2].shape
net2.reshape({input_layer2:(n2,c2,300,300)})        
plugin = IEPlugin(device="CPU") #you can choose CPU to call extensions
plugin.add_cpu_extension(location)
face_net = plugin.load(network=net2) #create an executable network
# model_hat = load_model("hat_32.h5")
model_frock = load_model("frock_32.h5")
model_glove = load_model("glove_32.model")
# model_edge=load_model('edge_128.h5')
...
# filehandler = logging.handlers.TimedRotatingFileHandler('./logs/mylog.log', when='M', interval=1, backupCount=5,
#                                                         encoding='utf-8')
...
def face_recog(image):
    ...
    if new_img.shape[0]==0 or new_img.shape[1]==0:
        return ('unknown')
#    cv2.imwrite('./logs/%s.jpg'%(count),new_img)
#    print(crop_img.shape)
    input_img = cv2.resize(new_img,(60,60), interpolation=cv2.INTER_AREA)
    input_img = np.transpose(input_img,(2,0,1))
    input_img = input_img[np.newaxis,:]
    cur_req = det_net.requests[0]
    input_blob = next(iter(net1.inputs))        
                #step6. VINO rateld. Just call the inference. Sooooo simple!
    det_net.requests[0].infer({input_blob: input_img})        
    out = cur_req.outputs['align_fc3']
#    print(out.shape)
    label=clf.predict_proba(out)[0]
...
def upload_log(label_dic,count_list,count,faces,img):
    ...
    #     mylog.info('Camer_ID:001;Worker_ID:%s;hat:%s;frock:%s;glove:%s;timestamp:%s;file_name:%s' % (
    #     faces, label_hat, label_frock, label_glove, time.time(), count))
    #     cv2.imwrite('./logs_img/%s.jpg' % (count), img)
    #     hat_flag = False
    # frock_flag = False
    # glove_flag = False
                ...
                for i in range(len(out)):
                    ...
                    if label_hat=='1':
                        if img.shape[0]==0 or img.shape[1]==0:
                            continue
                        # x2,y2,x3,y3=face_det(img)
                        # if x2==0 and x3==0 and y2==0 and y3==0:
                        #     continue
                        # img_face=img[y2:y3,x2:x3]
                        # if img_face.shape[0]==0 or img_face.shape[1]==0:
                        #     continue
                        # cv2.rectangle(img=img,pt1=(x2,y2),pt2=(x3,y3),color=(0,0,255))
                        # faces=face_recog(img_face)
                        # if faces==last_faces:
                        #     count_faces+=1
                        # else:
                        #     last_faces=faces
...
#                    if faces_label != "unknown":
#                        if last_faces == int(faces_label):
#                            count_faces += 1
#                        else:
#                            last_faces = int(faces_label)
#                            count_faces = 0
#                    else:
#                        count_faces = 0
