...
def draw_muti(image):
    ...
    with open(filename, "r") as f:
        load_dict = json.load(f) #多边形的顶点坐标
    self.pts = np.array(load_dict["base1"])
    cv2.polylines(image,[self.pts],True,(0,255,255), 2)  #画任意多边形
...
def put_data(img_name, my_result , img):
    header = {}
    cv2.imwrite('./logs_img/%s.jpg' % (img_name), img)
...
class Detect_my:
    def __init__(self):
        ...
        self.counter = []
        self.my_track_dict = {} #save the info of track_id
        self.track_smooth_dict = {} #smooth the imshow
        self.pts = [deque(maxlen=30) for _ in range(9999)]

        #deep_sort
        self.model_filename = 'model_data/market1501.pb'
        self.encoder = gdet.create_box_self.encoder(self.model_filename,batch_size=1)
        self.metric = nn_matching.NearestNeighborDistanceself.metric("cosine", self.max_cosine_distance, self.nn_budget)
        self.tracker = self.tracker(self.metric)
        self.list_file = open('detection_rslt.txt', 'w')
    ...
    def get_detect(self, id_video):
        t1 = threading.Thread(target=self.read_video)
        t2 = threading.Thread(target=self.process_my, args=(id_video,))
        t1.start()
        t2.start()
...
def detect_video2():
    ...
    if len(camer_list)>=2:
        number = camer_list[1]
    return Response(Detect_my.get_detect(number),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
